...
export default function SeniorPage() {
  ...
  // 상태 정의
  const [keyword, setKeyword] = useState('장애인 추천 일자리');
  ...
  const fetchVideos = async (kw) => {
    ...
    try {
      const res = await axios.get(
        `${BASE_URL}/disabled-category-search?keyword=${encodeURIComponent(kw)}`
      );
  ...
  const handleCategoryClick = (category) => {
    if (category.path) navigate(category.path);
      ...
      navigate(
        `/disabled-results/${category.id}?name=${encodeURIComponent(
          category.name
        )}`
      );
  };
